...
namespace ResourceDesigner.Controls.DockExtender
{
    /// <summary>
    ...
    internal sealed class Floaty : Form, IFloaty
    {
        ...
        private void MakeFloatable(DockState dockState, int offsetx, int offsety)
        {
            ...
            */
            ...
            if (_dockState.Container.Equals(_dockState.Handle))
            {
                if (_dockState.Handle is ToolStrip)
                {
                    var ts = _dockState.Handle as ToolStrip;

                    var minWidth = ts.Items.Cast<ToolStripItem>().Sum(c => c.Width + c.Padding.Left + c.Padding.Right + c.Margin.Left + c.Margin.Right);
                    if (minWidth > 0)
                        sz.Width = minWidth + 16;
